          ...
          {step === 'form' && (
            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input required type="text" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} className="w-full bg-[#0a0a12] border border-[rgba(0,212,255,0.2)] rounded p-2 text-white outline-none focus:border-[#00d4ff]" />
              </div>
              <div>
                <label className="block text-xs text-[#a0a8b8] mb-1">Last Name *</label>
                <input required type="text" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} className="w-full bg-[#0a0a12] border border-[rgba(0,212,255,0.2)] rounded p-2 text-white outline-none focus:border-[#00d4ff]" />
              </div>
              <div>
                ...
                <div className="flex">
                  ...
                  <input required type="text" maxLength={9} pattern="\d{9}" placeholder="555555555" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full bg-[#0a0a12] border border-[rgba(0,212,255,0.2)] rounded-r p-2 text-white outline-none focus:border-[#00d4ff]" />
                </div>
              </div>
              <div>
                <label className="block text-xs text-[#a0a8b8] mb-1">Email (Optional)</label>
                <input type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-[#0a0a12] border border-[rgba(0,212,255,0.2)] rounded p-2 text-white outline-none focus:border-[#00d4ff]" />
              </div>
              <div>
                <label className="block text-xs text-[#a0a8b8] mb-1">Order Notes (Optional)</label>
                <textarea rows={3} value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full bg-[#0a0a12] border border-[rgba(0,212,255,0.2)] rounded p-2 text-white outline-none focus:border-[#00d4ff]" />
              </div>
